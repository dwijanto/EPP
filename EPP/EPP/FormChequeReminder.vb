...
Public Class FormChequeReminder
    Dim DS As DataSet
    ...
    Sub DoWork()
        ...
        'get POHD with Criteria Accepted and EmailQtyConfirmation was sent
        ...
        '         " where qtyconfirmation = true and status = 2;" &
        ...
        sqlstr = "Select s.* from shop.paramhd  p left join shop.cutoff s on s.cutoffid = p.nvalue where p.paramname = 'Current Cutoff';" &
         ...
         " where qtyconfirmation = true and status = 2 and ph.cutoffid = p.nvalue;" &
         " select ph.stamp as mystamp, pd.*,i.refno,d.descriptionname from shop.podtl pd " &
         " left join shop.pohd ph on ph.pohdid = pd.pohdid" &
         " left join shop.item i on i.itemid = pd.itemid" &
         " left join shop.paramhd p on p.paramname = 'Current Cutoff'" &
        ...
        'Just keep sending email until employee pay or cutoff date changed
        'If Not sendManually Then
        '    bs.Filter = "chequereminder = False"
        'End If
        'Logger.log("bs.Filter =" & bs.Filter)
        'email will be sent even chequereminder = true as long as running manually
        If bs.List.Count > 0 Then
            ...
            For Each drv As DataRowView In bs.List
                ...
                If myemail.send(mymessage) Then
                    ...
                    emailsent += 1
                    sqlstr = "update shop.pohd ph set chequereminder = true where ph.pohdid = " & dr.Item("pohdid")
            ...
            Next
            ProgressReport(2, "Done. Email sent(" & emailsent & ")")
